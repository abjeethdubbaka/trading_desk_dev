...
export default function DayPanel({ day, trades = [], onClose }) {
  const dayTrades = useMemo(
    () => trades.filter((t) => { try { return isSameDay(new Date(t.entry_time || t.created_date), day); } catch { return false; } }),
    [trades, day]
  );
  ...
  return (
    <div className="bg-[#13131e] border border-white/10 rounded-2xl p-5 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2 overflow-y-auto max-h-72">
        {!dayTrades.length ? (
          <p className="text-white/30 text-sm text-center py-4">No trades</p>
        ) : dayTrades.map((t) => (
          <div key={t.id} className="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2">
            <div className="flex items-center gap-2">
              <span className={cn('text-xs font-bold px-1.5 py-0.5 rounded', t.direction === 'long' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400')}>
                {t.direction === 'long' ? 'L' : 'S'}
              </span>
              <div>
                <p className="text-sm font-semibold leading-none">{t.symbol}</p>
                <p className="text-[10px] text-white/40 mt-0.5">{t.setup_type || '—'}</p>
              </div>
            </div>
            <div className="text-right">
              <p className={cn('text-sm font-bold font-mono', (t.pnl || 0) >= 0 ? 'text-emerald-400' : 'text-red-400')}>
