    ...
    <div className="bg-[#13131e] border border-white/8 rounded-xl px-4 py-3">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {sequence.map((day, i) => (
          <div
            key={i}
            title={`${day.date}: ${day.pnl >= 0 ? '+' : ''}$${Math.round(day.pnl)}`}
            className={cn(
              'w-7 h-7 rounded-md flex items-center justify-center text-[10px] font-bold cursor-default transition-transform hover:scale-110',
