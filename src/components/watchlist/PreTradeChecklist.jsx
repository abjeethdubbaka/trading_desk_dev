...
export default function PreTradeChecklist({ onAllChecked }) {
  ...
  return (
    <div className={cn('rounded-xl border p-4 space-y-3 transition-all duration-300',
      allDone?'bg-emerald-500/8 border-emerald-500/25':'bg-[#13131e] border-white/8')}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {allDone
            ? <CheckCircle2 className="w-4 h-4 text-emerald-400" />
            : <AlertTriangle className="w-4 h-4 text-amber-400/70" />}
          <p className={cn('text-xs font-semibold uppercase tracking-wider',allDone?'text-emerald-400':'text-white/50')}>
            Pre-trade checklist
          </p>
        </div>
      ...
      <div className="space-y-2">
        {rules.map((rule,i)=>(
          <button key={i} onClick={()=>toggle(i)} className="flex items-start gap-2.5 w-full text-left group">
            ...
            <span className={cn('text-xs leading-relaxed transition-colors',
              checked[i]?'text-white/35 line-through':'text-white/65 group-hover:text-white/85')}>
              {rule}
            </span>
          </button>
        ))}
