...
import { computeCoreStats, filterByPeriod, buildEquityCurve, computeMaxDrawdown } from '@/lib/performanceMetrics';
...
function Delta({ diff, fmt, higherIsBetter=true }) {
  ...
  return <span className={cn('text-[10px] font-semibold flex items-center gap-0.5 mt-0.5',pos?'text-emerald-400':'text-red-400')}>{pos?<TrendingUp className="w-3 h-3"/>:<TrendingDown className="w-3 h-3"/>}{fmt(diff)} vs last mo.</span>;
}
...
export default function PeriodComparison({ trades=[], initialBalance=50000 }) {
  const curr = useMemo(()=>filterByPeriod(trades,'1M'),[trades]);
  const prev = useMemo(()=>{
    const now=new Date(),s=new Date(now),e=new Date(now);
    s.setMonth(now.getMonth()-2); e.setMonth(now.getMonth()-1);
    return trades.filter(t=>{ const d=new Date(t.entry_time||t.created_date||0); return d>=s&&d<e; });
  },[trades]);
  const cS=useMemo(()=>computeCoreStats(curr),[curr]);
  const pS=useMemo(()=>computeCoreStats(prev),[prev]);
  const cDD=useMemo(()=>computeMaxDrawdown(buildEquityCurve(curr,initialBalance)),[curr,initialBalance]);
  const pDD=useMemo(()=>computeMaxDrawdown(buildEquityCurve(prev,initialBalance)),[prev,initialBalance]);
