...
import { buildEquityCurve, computeMaxDrawdown, filterByPeriod } from '@/lib/performanceMetrics';
...
export default function EquityCurve({ trades=[], initialBalance=50000 }) {
  ...
  const start = curve[0]?.balance??initialBalance, end=curve[curve.length-1]?.balance??initialBalance;
    ...
    <div className="bg-[#13131e] border border-white/8 rounded-2xl p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {PERIODS.map(p=>(
            <button key={p} onClick={()=>setPeriod(p)} className={cn('text-xs px-3 py-1 rounded-md transition-all',period===p?'bg-purple-500/20 text-purple-300 border border-purple-500/30':'bg-white/5 text-white/40 hover:text-white/70')}>{p}</button>
          ))}
        </div>
      </div>
      <div className="h-52">
