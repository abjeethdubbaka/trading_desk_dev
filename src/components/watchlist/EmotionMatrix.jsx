...
import { computeEmotionStats } from '@/lib/performanceMetrics';
import { cn } from '@/lib/utils';

const COLORS = {
  confident:{dot:'bg-emerald-400',text:'text-emerald-400'},disciplined:{dot:'bg-blue-400',text:'text-blue-400'},
  neutral:{dot:'bg-white/40',text:'text-white/60'},nervous:{dot:'bg-amber-400',text:'text-amber-400'},
  fomo:{dot:'bg-orange-400',text:'text-orange-400'},revenge:{dot:'bg-red-400',text:'text-red-400'},
};
            ...
            {stats.map(row=>{
              ...
              return (
                <tr key={row.emotion} className="hover:bg-white/3 transition-colors">
                  ...
                  <td className={cn('py-2 px-3 text-right font-mono font-semibold',row.winRate>=50?'text-emerald-400':row.winRate>=40?'text-amber-400':'text-red-400')}>{row.winRate.toFixed(0)}%</td>
                  <td className={cn('py-2 px-3 text-right font-mono font-semibold',row.avgR>=1?'text-emerald-400':row.avgR>=0?'text-amber-400':'text-red-400')}>{row.avgR>=0?'+':''}{row.avgR.toFixed(1)}R</td>
                  <td className={cn('py-2 px-3 text-right font-mono font-semibold',row.avgPnL>=0?'text-emerald-400':'text-red-400')}>{row.avgPnL>=0?'+':''}${row.avgPnL.toFixed(0)}</td>
                  <td className={cn('py-2 pl-3 text-right font-mono',row.totalPnL>=0?'text-emerald-400/70':'text-red-400/70')}>{row.totalPnL>=0?'+':''}${row.totalPnL.toFixed(0)}</td>
                </tr>
              );
            })}
          </tbody>
