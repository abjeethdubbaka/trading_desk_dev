...
function readCache(id) {
  try { const c=JSON.parse(localStorage.getItem(CACHE_KEY)||'{}'); const h=c[id]; return h&&Date.now()-h.ts<TTL?h.data:null; } catch { return null; }
}
function writeCache(id, data) {
  try { const c=JSON.parse(localStorage.getItem(CACHE_KEY)||'{}'); c[id]={data,ts:Date.now()}; localStorage.setItem(CACHE_KEY,JSON.stringify(c)); } catch {}
}
...
export function useTradeReview() {
  ...
  const reviewTrade = useCallback(async (trade) => {
    ...
    if (cached) { setReviews(p=>({...p,[id]:cached})); return; }
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages:[{ role:'user', content:`You are a trading coach. Analyze this trade briefly.
...
{"grade":"A"|"B"|"C"|"D"|"F","verdict":"win"|"loss"|"breakeven","what_went_well":"<15 words>","what_to_improve":"<15 words>","key_lesson":"<20 words>","next_time":"<15 words>"}`
  ...
  const clearReview = useCallback((id) => {
    setReviews(p=>{ const n={...p}; delete n[id]; return n; });
    try { const c=JSON.parse(localStorage.getItem(CACHE_KEY)||'{}'); delete c[id]; localStorage.setItem(CACHE_KEY,JSON.stringify(c)); } catch {}
  }, []);
