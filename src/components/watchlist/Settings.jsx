...
export default function Settings() {
  ...
  const handle = key => e => { updateSettings({[key]:e.target.value}); triggerSave(); };

  const exportCSV = () => {
    const trades = JSON.parse(localStorage.getItem('trades')||'[]');
    const rows = [['date','symbol','direction','entry','exit','size','pnl','r_multiple','setup','emotions','followed_plan'],
      ...trades.map(t=>[(t.entry_time||t.created_date||'').slice(0,10),t.symbol||'',t.direction||'',t.entry_price||'',t.exit_price||'',t.position_size||'',t.pnl||'',t.r_multiple||'',t.setup_type||'',t.emotions||'',t.followed_plan||''])];
  ...
  const exportJSON = () => {
    const data={trades:JSON.parse(localStorage.getItem('trades')||'[]'),settings:JSON.parse(localStorage.getItem('userSettings')||'{}'),calcHistory:JSON.parse(localStorage.getItem('calcHistory')||'[]'),exportedAt:new Date().toISOString()};
    const a=document.createElement('a');
      ...
      <Tabs defaultValue="account">
        ...
        <TabsContent value="data" className="mt-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <button
              onClick={()=>{if(confirm('Delete ALL trades? This cannot be undone.')&&prompt('Type DELETE to confirm:')==='DELETE'){localStorage.removeItem('trades');window.dispatchEvent(new CustomEvent('trades-updated',{detail:{action:'reset'}}));alert('Done.');}}}
            ...
            <button
              onClick={()=>{if(confirm('Reset all settings to defaults?')){localStorage.removeItem('userSettings');window.location.reload();}}}
