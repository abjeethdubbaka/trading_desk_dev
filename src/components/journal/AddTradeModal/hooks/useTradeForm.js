...
export const useTradeForm = (initialData, userId = PLACEHOLDER_USER_ID, open = true) => {
  ...
  const initialFormData = useMemo(() => {
    ...
    return {
      ...
      stop_loss_used: initialData.stop_loss_used || '',
      exit_used: initialData.exit_used || '',
      entry_criteria_used: initialData.entry_criteria_used || '',
      stop_loss_move_used: initialData.stop_loss_move_used || '',
  ...
  const prepareForSubmission = useCallback(() => {
    ...
    const normalizedReflectionAnswers = {
      ...
      what_worked: normalizeKeywordList(reflectionAnswers.what_worked),
    ...
    const submissionData = {
      ...
      exit_used: formData.exit_used ? String(formData.exit_used).trim() : null,
      overall_rating: Number.isFinite(Number(formData.overall_rating))
        ? Number(formData.overall_rating)
        : null,
      stop_loss: formData.stop_loss ? parseFloat(formData.stop_loss) : null,
      stop_loss_used: formData.stop_loss_used ? String(formData.stop_loss_used).trim() : null,
      entry_criteria_used: formData.entry_criteria_used ? String(formData.entry_criteria_used).trim() : null,
      stop_loss_move_used: formData.stop_loss_move_used ? String(formData.stop_loss_move_used).trim() : null,
